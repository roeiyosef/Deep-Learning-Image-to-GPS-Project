...
class DualViewDataset(Dataset):
    def __init__(self, dataframe, root_dir, weak_transform, strong_transform=None, is_validation=False):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # יצירת תמונה שחורה במקרה של שגיאה כדי לא להקריס
            img = Image.new('RGB', (224, 224))

        # נרמול ה-GPS (בדיוק כמו באימון המקורי)
        gps = torch.tensor([
            (row['utm_x'] - CENTER_X) / GPS_SCALE,
            (row['utm_y'] - CENTER_Y) / GPS_SCALE
        ], dtype=torch.float32)

        # Label לקלסיפיקציה (Smart Zones)
        ...
        # טיפול מיוחד ללילה: לא עושים אוגמנטציות חזקות כדי לא להרוס את המידע
        if is_night:
...
def get_hard_negatives(embeddings, gps, safe_radius=0.2): # 0.2 מנורמל = 20 מטר
    """
    מוצא דוגמאות שהן קרובות במרחב ה-Embedding אבל רחוקות פיזית (GPS).
    """
    dist_emb = torch.cdist(embeddings, embeddings, p=2)
    dist_gps = torch.cdist(gps, gps, p=2)
    
    # מסכה: אנחנו רוצים רק דוגמאות שהן רחוקות פיזית (מעל 20 מטר)
    valid_mask = (dist_gps > safe_radius)
    
    # איפה שלא ולידי (קרוב מדי פיזית), נשים אינסוף כדי לא לבחור בזה
    dist_emb_masked = dist_emb.clone()
    dist_emb_masked[~valid_mask] = float('inf')
    
    # בחר את האינדקס עם המרחק הכי קטן (הכי דומה ויזואלית שהוא שלילי קשה)
    hard_neg_indices = dist_emb_masked.argmin(dim=1)
    
    return embeddings[hard_neg_indices]

# --- טרנספורמציות (Augmentations) ---
def get_transforms():
    ...
    # אוגמנטציה חזקה (ל-Positive ביום) - כולל מחיקה וסיבוב
    strong = transforms.Compose([
        transforms.RandomRotation(degrees=5),
        transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.1, hue=0.02),
        transforms.ToTensor(),
        transforms.Normalize(mean=CAMPUS_MEAN, std=CAMPUS_STD),
        transforms.RandomErasing(p=0.3, scale=(0.02, 0.2)) # הסתרה של חלקים
    ])
...
def train():
    ...
    # 1. טעינת הדאטה וסידור Sampler
    if not os.path.exists(CSV_PATH):
        raise FileNotFoundError(f"Missing {CSV_PATH}. Please download 'Preprocessed Data' from Drive.")
        
    full_df = pd.read_csv(CSV_PATH)
    
    # פיצול Train/Val (פשוט לפי ה-80/20 האחרונים או רנדומלי עם seed קבוע)
    # כאן אנחנו עושים רנדומלי כדי שרועי יוכל להריץ
    ...
    print("⚖️ Calculating Weighted Sampler (Balancing Zones)...")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for img_anc, img_pos, gps, label in loop:
            img_anc, img_pos = img_anc.to(DEVICE), img_pos.to(DEVICE)
            gps, label = gps.to(DEVICE), label.to(DEVICE)
        ...
        with torch.no_grad():
            for img, gps in val_loader:
                ...
                pred_gps, _, _ = model(img)
                dist = torch.norm((pred_gps - gps) * GPS_SCALE, dim=1)
                val_dist_sum += dist.sum().item()
        
        avg_val_dist = val_dist_sum / len(val_ds)
        print(f"🏁 Epoch {epoch}: Train Dist: {avg_train_dist:.2f}m | Val Dist: {avg_val_dist:.2f}m")
        
        # early stopping in case the val dist doesnt decrease in 15 epochs
